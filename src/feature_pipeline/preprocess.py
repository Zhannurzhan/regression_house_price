...
def normalize_metro_full(s: str) -> str:
    """Normalize metro_full values and remove trailing state suffixes."""
    if pd.isna(s):
        return s
    return normalize_city(str(s).split(",", maxsplit=1)[0])
...
def clean_and_merge(df: pd.DataFrame, metros_path: str | None = "data/raw/usmetros.csv") -> pd.DataFrame:
    ...
    df = df.copy()
    old_merge_cols = ["lat", "lng", "lat_x", "lng_x", "lat_y", "lng_y", "metro_full"]
    df = df.drop(columns=[col for col in old_merge_cols if col in df.columns])
